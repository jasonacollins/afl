...
Current-season ELO workflows start from a trained model artifact, apply offseason carryover when needed, then replay completed matches to update ratings before predicting or simulating future matches. Margin ELO replay uses shared helpers in `scripts/core/elo_core.py` so prediction generation, ELO history, and season simulation stay aligned.

Model workflows are organized around two prediction families: win-first and margin-first. Both publish `home_win_probability` and `predicted_margin`; win-first uses trained win ratings with a matching margin adapter, while margin-first derives win probability from margin output.
...
On the models page, win-first prediction derives the compatible margin adapter from the selected ratings artifact, and win-first training writes the matching pair of artifacts.
...
- Model artifacts keep trained-through metadata or filename conventions for compatibility; win-first margin adapters also record the win model they match.
