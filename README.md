...
- **Interactive ELO Chart**: Visualize team strength over time with round-by-round ELO ratings
...
- **ELO Service**: ELO rating data processing and chart generation
...
- **Round-based Timeline**: Shows ELO ratings at the start of each round (OR, 1-23, plus finals)
...
- **Year Selection**: Dropdown to view ELO charts for different AFL seasons
- **Visual Design**: Bold, vibrant colors with selected teams highlighted against faded gray background

### Usage

1. **View All Teams**: Default view shows all team ELO progressions
2. **Select Single Team**: Click on any team name in the legend to highlight only that team
3. **Select Multiple Teams**: Hold Ctrl/Cmd and click team names to select multiple teams
4. **Change Year**: Use the year dropdown to view different seasons
5. **Clear Selection**: Click the same team again to deselect, or click another team to switch

### Data Source

The ELO chart uses rating history data generated by the ELO prediction model. The data files are automatically processed to show:
- Team ratings at the start of each round
- Proper AFL round ordering (Opening Round, regular season rounds 1-23, finals)
- Current ratings displayed as the "next round" (e.g., Round 13 shows ratings going into Round 13)

### API Endpoints

- `GET /api/elo/years` - Returns available years for ELO data
- `GET /api/elo/ratings/:year` - Returns processed ELO rating data for visualization